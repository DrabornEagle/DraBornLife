...
export function LifeReportPanel({ lifeData, money }) {
  ...
  return (
    <View style={box}>
      <Text style={head} numberOfLines={2}>Antalya karar raporu</Text>
      <Text style={sub} numberOfLines={3}>Yaşam planında bugün hangi alan öne çıkıyor?</Text>
      <View style={hero}><Text style={heroTop}>Ana karar</Text><Text style={heroTitle} numberOfLines={2}>{main.title}</Text><Text style={heroText} numberOfLines={3}>{main.text}</Text></View>
...
function Line({ row }) { return <View style={line}><View style={{ flex: 1, paddingRight: 8 }}><Text style={lineTitle} numberOfLines={1}>{row.title}</Text><Text style={lineText} numberOfLines={2}>{row.text}</Text></View><Text style={scoreText} numberOfLines={1}>{row.value}</Text></View>; }
...
const hero = { marginTop: 10, padding: 13, borderRadius: 20, backgroundColor: '#102A35' };
const heroTop = { color: '#FFB347', fontSize: 11, fontWeight: '900' };
const heroTitle = { marginTop: 5, color: '#FFFFFF', fontSize: 17, lineHeight: 22, fontWeight: '900' };
...
const scoreText = { width: 34, textAlign: 'right', color: '#FF7A59', fontSize: 17, lineHeight: 21, fontWeight: '900' };
