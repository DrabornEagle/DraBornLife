...
export function ShoppingPriorityPanel({ items, money }) {
  ...
  return (
    <View style={panel}>
      <Text style={title}>Öncelik puanı</Text>
      <Text style={note}>Alınacak kalemler önem ve hazır olma durumuna göre sıralanır.</Text>
...
function Row({ item, index, money }) {
  return <View style={row}><Text style={rank}>{index + 1}</Text><View style={{ flex: 1 }}><Text style={rowTitle}>{item.title}</Text><Text style={rowText}>{item.category} • Kalan: {money(item.remaining)}</Text></View><Text style={score}>{item.priorityScore}</Text></View>;
}
...
const row = { marginTop: 9, padding: 12, borderRadius: 18, backgroundColor: '#F8FFFF', borderWidth: 1, borderColor: '#BEEDEF', flexDirection: 'row', alignItems: 'center' };
