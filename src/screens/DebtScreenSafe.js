...
export function DebtScreenSafe({ lifeData, onSave }) {
  ...
  const summary = useMemo(() => {
    ...
    const targetDate = nextTargetDate(debts) || '2026-10-31';
    const daysLeft = daysTo(targetDate);
    const weeksLeft = Math.max(1, Math.ceil(daysLeft / 7));
    const monthsLeft = Math.max(1, Math.ceil(daysLeft / 30));
    return { total, paid, left, completed, targetDate, daysLeft, weeklyNeed: Math.ceil(left / weeksLeft), monthlyNeed: Math.ceil(left / monthsLeft) };
  }, [debts]);
  ...
  function saveDebt() {
    ...
    const next = { id: editingId || `debt_${Date.now()}`, title, totalDebt: total, paidAmount: paid, targetFinishDate: date, note: form.note.trim(), paymentHistory: old?.paymentHistory || [], updatedAt: new Date().toISOString() };
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#F4FBF9' }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 18, paddingBottom: 190 }}>
      <Header title="Borç kapatma planı" text="Borçları hedef tarihe göre haftalık ve aylık ödeme planıyla takip et." />
      <NoticeBox message={notice.text} type={notice.type} />
      <CurrencySelector lifeData={lifeData} onSave={onSave} />

      <Panel title="Borç özeti" note="Kalan borç ve ödeme temposu seçili para birimine göre gösterilir.">
      ...
      <Panel title="Ödeme planı" note={`En yakın hedef: ${summary.targetDate} • ${summary.daysLeft} gün kaldı`}>
        <View style={{ flexDirection: 'row', marginTop: 10 }}><Card label="Haftalık ödeme" value={money(summary.weeklyNeed)} /><Card label="Aylık ödeme" value={money(summary.monthlyNeed)} /></View>
        <Text style={{ marginTop: 10, color: '#315661', fontSize: 13, lineHeight: 19, fontWeight: '800' }}>{summary.left <= 0 ? 'Tebrikler, kayıtlı borçların kapanmış görünüyor.' : 'Bu tempo kalan toplam borcu hedef tarihe kadar kapatmak için öneridir.'}</Text>
      </Panel>
      ...
      <Panel title="Borç kartları" note="Her borç kendi hedef tarihine göre ödeme önerisi gösterir.">
        {debts.length === 0 ? <EmptyStateCard icon="📉" title="Henüz borç kaydı yok" text="Borç eklediğinde toplam, ödenen, kalan ve önerilen ödeme planı burada görünür." /> : debts.map(x => <DebtRow key={x.id} item={x} onEdit={editDebt} onDelete={removeDebt} onPay={addPayment} money={money} />)}
      </Panel>
    </ScrollView>
  );
}
...
function DebtRow({ item, onEdit, onDelete, onPay, money }) {
  ...
  const target = item.targetFinishDate || '2026-10-31'; const daysLeft = daysTo(target); const weekly = Math.ceil(left / Math.max(1, Math.ceil(daysLeft / 7))); const monthly = Math.ceil(left / Math.max(1, Math.ceil(daysLeft / 30)));
  return <View style={{ marginTop: 9, padding: 14, borderRadius: 20, backgroundColor: done ? '#E9FFF5' : '#F8FFFF', borderWidth: 1, borderColor: '#BEEDEF' }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><Text style={{ flex: 1, paddingRight: 8, color: '#102A35', fontSize: 15, lineHeight: 20, fontWeight: '900' }}>{item.title}</Text><Text style={{ maxWidth: 120, textAlign: 'right', color: done ? '#128C7E' : '#FF5E7E', fontSize: 14, lineHeight: 18, fontWeight: '900' }} numberOfLines={2}>{done ? 'Kapandı' : money(left)}</Text></View><Text style={{ marginTop: 6, color: '#315661', fontSize: 12, lineHeight: 17, fontWeight: '800' }}>Ödenen: {money(paid)} / {money(total)} • Hedef: {target}</Text><View style={{ height: 10, marginTop: 10, borderRadius: 20, backgroundColor: '#CFECEE', overflow: 'hidden' }}><View style={{ width: `${percent}%`, height: '100%', backgroundColor: done ? '#128C7E' : '#2DE2E6' }} /></View><View style={{ flexDirection: 'row', marginTop: 10 }}><PlanBox label="Haftalık" value={done ? 'Tamam' : money(weekly)} /><PlanBox label="Aylık" value={done ? 'Tamam' : money(monthly)} /></View><Text style={{ marginTop: 10, color: '#315661', fontSize: 12, fontWeight: '900' }}>Ödeme ekle</Text><View style={{ flexDirection: 'row', marginTop: 7 }}><TextInput value={payment} onChangeText={setPayment} placeholder="Ödeme tutarı" keyboardType="numeric" placeholderTextColor="#7C969D" style={{ flex: 1, padding: 12, borderRadius: 16, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#BEEDEF', color: '#102A35', fontWeight: '800' }} /><TouchableOpacity onPress={() => { onPay(item.id, payment); setPayment(''); }} style={{ marginLeft: 8, paddingHorizontal: 14, borderRadius: 16, backgroundColor: '#2DE2E6', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#06202A', fontSize: 13, fontWeight: '900' }}>Ekle</Text></TouchableOpacity></View>{(item.paymentHistory || []).length > 0 && <Text style={{ marginTop: 8, color: '#315661', fontSize: 12, fontWeight: '800' }}>Son ödeme: {money(item.paymentHistory[0].amount)}</Text>}<View style={{ flexDirection: 'row', marginTop: 10 }}><Small label="Düzenle" active onPress={() => onEdit(item)} /><Small label="Sil" danger onPress={() => onDelete(item.id)} /></View></View>;
}
function PlanBox({ label, value }) { return <View style={{ flex: 1, marginRight: 8, padding: 10, borderRadius: 16, backgroundColor: '#E9FAFA', borderWidth: 1, borderColor: '#BEEDEF' }}><Text style={{ color: '#315661', fontSize: 11, fontWeight: '900' }}>{label}</Text><Text style={{ marginTop: 4, color: '#102A35', fontSize: 13, fontWeight: '900' }} numberOfLines={2}>{value}</Text></View>; }
...
function daysTo(value) { const date = new Date(`${value}T00:00:00`); if (Number.isNaN(date.getTime())) return 1; const now = new Date(); const today = new Date(now.getFullYear(), now.getMonth(), now.getDate()); return Math.max(1, Math.ceil((date.getTime() - today.getTime()) / 86400000)); }
function nextTargetDate(debts) { const active = debts.filter(x => Math.max(0, n(x.totalDebt) - n(x.paidAmount)) > 0 && validDate(x.targetFinishDate)).map(x => x.targetFinishDate).sort(); return active[0]; }
