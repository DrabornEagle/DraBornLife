...
export function MoneyScreenSafe({ lifeData, onSave }) {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#DFF5F6' }} contentContainerStyle={{ padding: 18, paddingBottom: 150 }}>
      <Header title="Gelir-Gider" text="Kayıt ekle, düzenle, sil ve güvenli veri girişi yap." />
      <NoticeBox message={notice.text} type={notice.type} />
      <View style={{ flexDirection: 'row', marginTop: 14 }}><Card label="Gelir" value={formatTRY(totals.income)} /><Card label="Gider" value={formatTRY(totals.expense)} /></View>
      <View style={{ flexDirection: 'row', marginTop: 8 }}><Card label="Net" value={formatTRY(totals.net)} /><Card label="Birikime" value={formatTRY(totals.saving)} /></View>

      <Panel title={editingId ? 'Kaydı düzenle' : 'Yeni kayıt'}>
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}><TypeButton label="Gelir" active={form.type === 'income'} onPress={() => setField('type', 'income')} /><TypeButton label="Gider" active={form.type === 'expense'} onPress={() => setField('type', 'expense')} /></View>
      ...
      <Panel title="Filtreler"><Chips items={typeFilters.map(x => x.label)} active={typeFilters.find(x => x.key === typeFilter)?.label || 'Tümü'} onPick={label => setTypeFilter(typeFilters.find(x => x.label === label)?.key || 'all')} /><Chips items={['Tümü', ...categories]} active={categoryFilter} onPick={setCategoryFilter} /></Panel>

      <Text style={{ color: '#102A35', fontSize: 22, fontWeight: '900', marginTop: 16 }}>Son işlemler</Text>
      <Text style={{ marginTop: 4, color: '#315661', fontSize: 13, fontWeight: '700' }}>{visible.length} kayıt gösteriliyor</Text>
      {visible.length === 0 ? <Text style={{ marginTop: 8, color: '#315661', fontSize: 14, fontWeight: '700' }}>Bu filtreye uygun kayıt yok.</Text> : visible.map(e => <Row key={e.id} entry={e} onEdit={editEntry} onDelete={removeEntry} />)}
    </ScrollView>
  );
}
...
function TypeButton({ label, active, onPress }) { return <TouchableOpacity onPress={onPress} style={{ flex: 1, paddingVertical: 13, borderRadius: 18, backgroundColor: active ? '#2DE2E6' : '#CFECEE', alignItems: 'center' }}><Text style={{ color: '#06202A', fontSize: 15, fontWeight: '900' }}>{label}</Text></TouchableOpacity>; }
...
function Row({ entry, onEdit, onDelete }) { const inc = entry.type === 'income'; return <View style={{ marginTop: 9, padding: 14, borderRadius: 18, backgroundColor: '#E9FAFA', borderWidth: 1, borderColor: '#BEEDEF' }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><Text style={{ flex: 1, color: '#102A35', fontSize: 15, fontWeight: '900' }}>{entry.title}</Text><Text style={{ color: inc ? '#128C7E' : '#FF5E7E', fontSize: 15, fontWeight: '900' }}>{inc ? '+' : '-'} {formatTRY(entry.amount)}</Text></View><Text style={{ marginTop: 5, color: '#315661', fontSize: 12, fontWeight: '800' }}>{inc ? 'Gelir' : 'Gider'} • {entry.category}</Text>{inc && n(entry.savingPartAmount) > 0 && <Text style={{ marginTop: 5, color: '#128C7E', fontSize: 12, fontWeight: '800' }}>Birikime: {formatTRY(entry.savingPartAmount)}</Text>}<View style={{ flexDirection: 'row', marginTop: 10 }}><MainButtonSmall label="Düzenle" color="#2DE2E6" onPress={() => onEdit(entry)} /><MainButtonSmall label="Sil" color="#FFD0D8" text="#7A1E2B" onPress={() => onDelete(entry.id)} /></View></View>; }
