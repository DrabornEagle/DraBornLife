  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#F3F6F8' }} showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 18, paddingBottom: 150 }}>
      ...
      <Panel title="Ev kurulum odaları" subtitle="Oda seç, eşya ekle, tamamlandı durumunu takip et.">
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>{rooms.map((room) => <Chip key={room.id} label={room.title} active={selectedRoom.id === room.id} onPress={() => { setSelectedRoomId(room.id); clearRoomForm(); }} />)}</ScrollView>
        {rooms.map((room) => { const items = room.items || []; const done = items.filter((item) => item.isCompleted).length; return <Line key={room.id} left={room.title} right={`%${percent(done, items.length)}`} note={`${done}/${items.length} tamam`} />; })}
      </Panel>

      <Panel title={`${selectedRoom.title || 'Oda'} eşyaları`} subtitle="Bu oda için alınacak veya kurulacak eşyaları ekle.">
      ...
      <Panel title="Aile aktivite bütçesi" subtitle="Aquapark, sahil, yemek, gezi ve tatil hedeflerini bütçeyle takip et.">
        ...
        {activities.length === 0 ? <EmptyText text="Henüz aktivite yok. Aile keyfi için ilk hedefi ekle." /> : activities.map((item) => <ActivityItem key={item.id} item={item} onToggle={toggleActivity} onEdit={editActivity} onDelete={deleteActivity} money={money} />)}
      ...
      <Panel title="Özel hedefler" subtitle="Özel hedefler alt menüdeki Hedef sekmesinden eklenir ve yönetilir.">
        {customGoals.length === 0 ? <EmptyText text="Henüz özel hedef yok. Alt menüden Hedef sekmesine girerek ilk hedefini ekleyebilirsin." /> : customGoals.map((item) => <Line key={item.id} left={item.title} right={item.isCompleted ? 'Tamam' : 'Aktif'} note={`${item.category || 'hedef'} - ${money(item.estimatedBudget)}`} />)}
      </Panel>
    </ScrollView>
  );
}
...
function Line({ left, right, note }) { return <View style={{ paddingVertical: 10, borderTopWidth: 1, borderTopColor: '#E3EAF0' }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><Text style={{ flex: 1, color: '#102A35', fontSize: 14, fontWeight: '900' }}>{left}</Text><Text style={{ color: '#FF7A59', fontSize: 13, fontWeight: '900' }}>{right}</Text></View><Text style={{ marginTop: 3, color: '#315661', fontSize: 12, fontWeight: '800' }}>{note}</Text></View>; }
function Chip({ label, active, onPress }) { return <TouchableOpacity onPress={onPress} style={{ marginRight: 8, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 18, backgroundColor: active ? '#2DE2E6' : '#E8F1F2' }}><Text style={{ color: '#06202A', fontSize: 12, fontWeight: '900' }}>{label}</Text></TouchableOpacity>; }
...
function EmptyText({ text }) { return <Text style={{ marginTop: 12, color: '#315661', fontSize: 14, lineHeight: 20, fontWeight: '800' }}>{text}</Text>; }
function RoomItem({ item, onToggle, onEdit, onDelete, money }) { return <View style={itemCard}><View style={row}><Text style={itemTitleStyle}>{item.title}</Text><Text style={{ color: item.isCompleted ? '#128C7E' : '#FF7A59', fontSize: 12, fontWeight: '900' }}>{item.isCompleted ? 'Tamam' : 'Bekliyor'}</Text></View><Text style={itemSub}>{money(item.estimatedPrice)} {item.note ? `- ${item.note}` : ''}</Text><View style={btnRow}><Small label="Tamam" onPress={() => onToggle(item.id)} active={item.isCompleted} /><Small label="Düzenle" onPress={() => onEdit(item)} active /><Small label="Sil" onPress={() => onDelete(item.id)} danger /></View></View>; }
function ActivityItem({ item, onToggle, onEdit, onDelete, money }) { const left = Math.max(0, n(item.estimatedPrice) - n(item.savedAmount)); return <View style={itemCard}><View style={row}><Text style={itemTitleStyle}>{item.title}</Text><Text style={{ color: item.isCompleted ? '#128C7E' : '#FF7A59', fontSize: 12, fontWeight: '900' }}>{item.isCompleted ? 'Tamam' : money(left)}</Text></View><Text style={itemSub}>{item.category} - Biriken: {money(item.savedAmount)} / {money(item.estimatedPrice)}</Text>{!!item.note && <Text style={itemSub}>{item.note}</Text>}<View style={btnRow}><Small label="Tamam" onPress={() => onToggle(item.id)} active={item.isCompleted} /><Small label="Düzenle" onPress={() => onEdit(item)} active /><Small label="Sil" onPress={() => onDelete(item.id)} danger /></View></View>; }
function PlaceItem({ item, onToggle, onEdit, onDelete, money }) { const visited = item.status === 'visited' || item.status === 'done'; return <View style={itemCard}><View style={row}><Text style={itemTitleStyle}>{item.title}</Text><Text style={{ color: visited ? '#128C7E' : '#FF7A59', fontSize: 12, fontWeight: '900' }}>{visited ? 'Gidildi' : 'Planlı'}</Text></View><Text style={itemSub}>{item.type} - {item.area} - {money(item.estimatedBudget)}</Text>{!!item.familyNote && <Text style={itemSub}>{item.familyNote}</Text>}<View style={btnRow}><Small label={visited ? 'Planlı' : 'Gidildi'} onPress={() => onToggle(item.id)} active={visited} /><Small label="Düzenle" onPress={() => onEdit(item)} active /><Small label="Sil" onPress={() => onDelete(item.id)} danger /></View></View>; }
